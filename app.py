        ...
        try:
            ...
            if st.sidebar.button("🚀 Start Processing", type="primary"):
                process_data(temp_file_path, target_column, dropped_columns, untouched_columns,
                           type_dataset, int(sampling) if classification == 1 else 0, classification, strategy_sample, df)
            ...
            with analysis_tab3:
                ...
                if len(numerical_df.columns) > 1:
                    ...
                    try:
                        shape_info, vif_df, messages = datainfo.filtered_correlation_matrix(df)
...
def process_data(file_path, target_column, dropped_columns, untouched_columns,
                type_dataset, sampling, classification, strategy_sample, original_df):
    ...
    try:
        ...
        with result_tab2:
            ...
            try:
                original_info, processed_info = datainfo.display(file_path, processed_df)
        ...
        with result_tab4:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"processed_{uploaded_file.name}" if 'uploaded_file' in locals() else "processed_data.csv",
                mime="text/csv"
            )
