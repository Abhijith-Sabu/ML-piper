...
def initial_check_dt(file_path: str, target_variable: str, columns_to_drop: List[str]) -> Tuple[Optional[pd.DataFrame], List[str]]:
    """
        ...
        @parameters:
            ...
            columns_to_drop : List[str] 
                ...
                would generally be columns that have to do with IDs.
        
        @return:

            Tuple[Optional[pd.DataFrame], List[str]] 
                It will return the modified dataframe and messages if the function is successful.
    ...
    if file_path.lower().endswith('.csv'):
        ...
        if id_drop:
            ...
            messages.append(f"Dropped untouched columns: {', '.join(id_drop)}")
        ...
        if drop_columns:
            messages.append(f"Dropped columns with high missing data: {list(drop_columns)}")
        ...
        if target_variable in df_columns:
            messages.append(f"The target variable '{target_variable}' is present in the DataFrame.")
        else:
            raise Exception (f"The target variable '{target_variable}' is missing in the DataFrame.")
        
        messages.append("Initial checks and changes made")
        return df, messages
    
    else:
        raise Exception("Oops :( The file is not a CSV file!")

def cross_sectional_imputation(cross_sectional_df: pd.DataFrame, target_name: str) -> Tuple[pd.DataFrame, dict]:
    """
        ...
        @return:
            ...
            Tuple[pd.DataFrame, dict] 
                It will return the imputed dataframe and null counts info if the function is successful.
    ...
    return df, {'before': null_before, 'after': null_after}

    
def time_series_imputation(time_series_df: pd.DataFrame, target_name: str) -> Tuple[pd.DataFrame, dict]:
    """
...
def handle_imbalanced_data(df: pd.DataFrame, target_variable: str, strategy = "smote", k_neighbors=2, untouched_columns: List[str]=[]) -> Tuple[pd.DataFrame, dict]:
    """
        ...
        @return:
            ...
            Tuple[pd.DataFrame, dict]
                The balanced dataframe and distribution info after following the sampling stratergy.
    ...
    distribution_before = df[target_variable].value_counts().to_dict()
    
    if strategy == "smote":

        features, target = separate_target_column(df, target_variable)

        features = encoding(features, untouched_columns)[0]
    ...
    distribution_after = df_balanced[target_variable].value_counts().to_dict()
    
    return df_balanced, {'before': distribution_before, 'after': distribution_after}
...
def remove_outliers(df: pd.DataFrame) -> Tuple[pd.DataFrame, dict]:
    """
        ...
        @return:
            ...
            Tuple[pd.DataFrame, dict]
                The dataframe after outlier removal and count info.
    ...
    count_info = {
        ...
        'after_removal': len(df_pre_processed),
        'removed': len(df_original) - len(df_pre_processed)
    }

    return df_pre_processed, count_info
...
def encoding(features: pd.DataFrame, untouched_columns: List[str]=[]) -> Tuple[pd.DataFrame, List[str]]:
    """
        ...
        @parameters:
            ...
            untouched_columns : List[str]
                The list consists of all the columns that should not be encoded.
        
        @return:
            Tuple[pd.DataFrame, List[str]]:
                The dataframe consisting of all the encoded features and messages. 
    """
    messages = []
    messages.append(f"Before encoding: {list(features.columns)}")
    ...
    for column in features.columns:
        if features[column].dtype == "object" and column not in untouched_columns:
            if features[column].nunique() > 3:
                ...
                messages.append(f"Label encoded: {column}")
            else:
                ...
                messages.append(f"One-hot encoded: {column}")
    
    messages.append(f"After encoding: {list(features.columns)}")

    return features, messages


def feature_scaling(features: pd.DataFrame, unscale_columns: List[str]) -> Tuple[pd.DataFrame, dict]:
    """
        ...
        @return:
            ...
            Tuple[pd.DataFrame, dict]:   
                The dataframe containing the scaled features and scaling info.
    ...
    scaling_info = {
        'minmax': Minmaxscaler_algorithms,
        'standard': Standardscaler_algorithms
    }
...
def preprocess_pipeline(file_path: str, target_column: str, dropped_columns: List[str]=[], untouched_columns: List[str]=[], type_dataset: int=0, sampling: int=0, classfication: int=1, strategy_sample="smote"):
    """
        ...
        @parameters:
            ...
            strategy_sample: str: default = SMOTE
                The name of the sampling stratergy:
                    - oversampling 
                    - undersampling 
                    - smote 
        ...
        @return:
            ...
            Tuple[pd.DataFrame, List[str]]:
                The processed dataframe and processing messages.

    """
    all_messages = []
    
    all_messages.append("Starting Initial Check...")
    df, initial_messages = initial_check_dt(file_path, target_column, dropped_columns)
    all_messages.extend(initial_messages)
    all_messages.append("Initial check completed")
    
    all_messages.append("Starting Imputation...")
    if not type_dataset:
        df, impute_info = cross_sectional_imputation(df, target_column)
        all_messages.append("Cross Sectional Imputation Done")
    elif type_dataset == 1:
        df, impute_info = time_series_imputation(df, target_column)
        all_messages.append("Time Series Imputation Done")

    if sampling:
        if classfication:
            if df[target_column].nunique() > 2:
                all_messages.append("Balancing Operations not supported for Multi-class Classification")
            else:
                all_messages.append("Handling Imbalanced Data...")
                df, balance_info = handle_imbalanced_data(df, target_column, strategy_sample, untouched_columns=untouched_columns)
                if strategy_sample == "smote":            
                    all_messages.append("Balanced and Encoded the data")
                else:
                    all_messages.append("Balanced the data")
        else:
           all_messages.append("Balancing Operations not supported for Regression Models")

    all_messages.append("Removing Outliers...")
    df, outlier_info = remove_outliers(df)
    all_messages.append("Removed Outliers")
    
    all_messages.append("Separating Input and Output...")
    remaining_df, target = separate_target_column(df, target_column)
    all_messages.append("Separated the input and output")
    
    
    if strategy_sample != "smote" or df[target_column].nunique() > 2:
        all_messages.append("Encoding Data...")
        remaining_df, encoding_messages = encoding(remaining_df, untouched_columns)
        all_messages.extend(encoding_messages)
        all_messages.append("Encoded the data")
    
    all_messages.append("Feature Scaling...")
    preprocessed_df, scaling_info = feature_scaling(remaining_df, untouched_columns)
    all_messages.append("Feature Scaling Done")
    
    all_messages.append("Combining Feature and Target...")
    combined_df = combine(preprocessed_df, target)
    all_messages.append("Combining Features Done")
    
    return combined_df, all_messages